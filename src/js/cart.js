...
const cartButton = document.querySelector('.store__cart-button'); // кнопка корзины
...
// ========== изменяем показания счетчика корзины ========
...
cartForm.addEventListener('submit', async (event) => {
  // тут остановились
  event.preventDefault(); // запретили браузеру обновлять страницу

  // у input name="store" !!!
  const storeId = cartForm.store.value;
  const cartItems = JSON.parse(localStorage.getItem('cartItems') || "[]");

  // перекрутили объект - на серверу нужен quantity....
